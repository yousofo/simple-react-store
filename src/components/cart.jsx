...
const Cart = () => {
  ...
  useEffect(() => {
    setCarts(JSON.parse(window.localStorage.getItem("cart")))
  }, [])
  ...
  const removeProduct = (id) => {
    let updatedCart = carts.filter(item => item.id != id)
                ...
                <div key={i} className="flex items-center hover:bg-gray-100 py-4">
                  <div className="flex w-3/5 sm:w-2/5 justify-between gap-2">
                    <div className="w-20 h-24">
                      <img className="w-full h-full object-cover" src={cart?.thumbnail} alt={cart?.title} />
                    </div>
                    <div className="flex flex-col w-min justify-between flex-grow">
                  ...
                  <div className="flex justify-center w-1/5">
                    ...
                    <span className="mx-2 border text-center w-8" type="text">{cart?.quantity}</span>
                  ...
                  <span className="text-center w-1/5 font-semibold hidden sm:block text-sm">${cart?.price}</span>
